...
{
	FILE* pfRead = fopen("contact.txt", "rb");//二进制只读
	...
	PeoInfo tmp = { 0 };  
	//创建一个中间变量用于存放fread读取到的数据

	//当fread读取成功的时候，其返回值不等于0
	...
	//我要把从文件中读取到的信息存放到通讯录中
	//但是在此之前，需要判断通讯录是否有足够空间存放数据
	//所以需要先将读取到的数据存放到一个中间变量中
	//然后判断通讯录的容量是否足够
	//如果容量不够，那么需要扩容
...
int SaveContact(const Contact* pc)
{
	...
	//打开文件
	FILE* pfWrite = fopen("contact.txt", "wb");//二进制写入
